 ...
 * @param {any[]} transactions
 * @returns {string}
 * @returns {{ text: string, monthlyBalances: { month: string, balance: number }[] }}
 */
function buildBalanceSummary(transactions) {
    ...
    for (const tx of transactions) {
        ...
        if (tx.month) {
            if (!monthlyMap[tx.month]) monthlyMap[tx.month] = { income: 0, expense: 0 };
            if (tx.type === "income") {
                monthlyMap[tx.month].income += tx.amount;
            } else {
                monthlyMap[tx.month].expense += tx.amount;
            }
        }
    }
    ...
    let cumulative = 0;
    const monthlyBalances = sortedMonths.map((m) => {
        cumulative += monthlyMap[m].income - monthlyMap[m].expense;
...
/**
 * \u6b8b\u9ad8\u63a8\u79fb\u30c1\u30e3\u30fc\u30c8URL\u3092\u751f\u6210
 * @param {{ month: string, balance: number }[]} monthlyBalances
 * @param {string} baseUrl
 * @returns {string}
 */
function buildBalanceChartUrl(monthlyBalances, baseUrl) {
    const months = monthlyBalances.map((mb) => mb.month).join(",");
    const balances = monthlyBalances.map((mb) => mb.balance).join(",");
    return `${baseUrl}/api/balance-chart?months=${encodeURIComponent(months)}&balances=${encodeURIComponent(balances)}`;
}
